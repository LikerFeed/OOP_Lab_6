...
void MatrixDeterminant::OnCreate(HWND hWnd)
{
...
int MatrixDeterminant::GetFromClipboard(HWND hWnd, long maxsize)
{
	HGLOBAL hglb;
	LPTSTR lptstr;
	long size, res;
...
int** MatrixDeterminant::GetMatrix(std::string text)
{
...
void MatrixDeterminant::GetSubmatrix(int** src, int** dest, int N, int row, int col)
{
	int di = 0;
	int dj = 0;
...
int MatrixDeterminant::GetDet(int** matrix, int size)
{
	int det = 0;
	int degree = 1;
...
void MatrixDeterminant::OnPaint(HWND hWnd, HDC hdc)
{
